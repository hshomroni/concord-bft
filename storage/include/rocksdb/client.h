...
class Client : public concord::storage::IDBClient {
 ...
 private:
  ...
  class RocksDbStorageMetrics : public StorageMetricsBase {
    static constexpr size_t update_metrics_interval_msec_ = 100;  // every 100msec
    static constexpr size_t update_mem_usage_metrics_factor_ =
        600;  // update_metrics_interval_msec_ * 600 = every 1 minute
   ...
   public:
    ...
    // update the metrics every update_metrics_interval_msec_
   ...
   private:
    ...
    std::map<::rocksdb::MemoryUtil::UsageType, concordMetrics::AtomicGaugeHandle> rocksdb_to_concord_metrics_map{
