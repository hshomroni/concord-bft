...
void Client::RocksDbStorageMetrics::setMetricsDataSources(std::shared_ptr<::rocksdb::SstFileManager> sourceSstFm,
  ...
  update_metrics_ =
      std::make_unique<concord::util::PeriodicCall>([this]() { updateMetrics(); }, update_metrics_interval_msec_);
}
...
void Client::RocksDbStorageMetrics::updateMetrics() {
  ...
  if (entry_count++ % update_mem_usage_metrics_factor_ == 0) {
    // enter here only once every update_mem_usage_metrics_factor_ calls of updateMetrics()
...
void Client::RocksDbStorageMetrics::updateDBMemUsageMetrics() {
  ...
  for (auto &[rocks_db_usage_type_name, concord_metric_handle] : rocksdb_to_concord_metrics_map) {
